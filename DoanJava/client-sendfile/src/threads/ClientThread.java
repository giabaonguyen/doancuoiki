...
//	                        SoundEffect.MessageReceive.play(); //  Play Audio clip
	                        ...
	                        ChatSingle cs = main.getChatSingle(frm);
							if(cs!=null) {
								cs.showFriendChat(msg);
							}else {
							cs= new ChatSingle(frm, main);
							cs.setVisible(true);
							cs.showFriendChat(msg);
							main.addToChatSingleList(cs);
							}
	                        break;
	                    ...
	                    case "CMD_ONLINE":
	                        ...
	                        while(st.hasMoreTokens()){
	                         ...
	                         if(!list.equals(main.username)){
	                                online.add(list);
	                            }
	                        }
	                        main.loadListFriend(online);
	                        break;
	                    ...
	                    case "CMD_FILE_XD":  // Format:  CMD_FILE_XD [sender] [receiver] [filename]
	                        ...
	                        //if(confirm == 0){ // client accepted the request, then inform the sender to send the file now
	                            ...
	                            try {
	                                ...
	                                /*  Run Thread for this   */
	                                new Thread(new ReceivingFileThread(main, fSoc,fname)).start();
	                            } catch (IOException e) {
